...
@Service
public class ServicioModificarJugador {
    private static final String MENSAJE_NO_EXISTE = "No existe el Jugador con los datos ingresados";

    private final RepositorioJugador repositorioJugador;

    public ServicioModificarJugador(RepositorioJugador repositorioJugador) {
        this.repositorioJugador = repositorioJugador;
    }

    public Long ejecutar(Jugador jugador, Long codigo) {

        if(this.repositorioJugador.consultarPorId(codigo)== null) {
            throw new IllegalStateException(MENSAJE_NO_EXISTE);
        }
        return this.repositorioJugador.modificar(jugador,codigo);
    }
}
